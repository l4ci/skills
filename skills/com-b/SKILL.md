...
State the target behaviour as a concrete action (who does what, when, where) and the specific actor expected to perform it, since each component is judged against a concrete action by a concrete actor. If BEHAVIOUR or ACTOR is loose, sharpen it now; carry the sharpened definition into every subagent prompt.
...
One agent merges the six assessments into a diagnosis and an intervention plan, finding the real barrier rather than restating the components:
...
- **Find the barrier, do not assume it.** Locate which of Capability, Opportunity, and Motivation is the binding constraint. Assuming it is motivation is the classic, costly error.
