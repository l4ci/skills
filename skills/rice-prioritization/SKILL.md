...
Ranks a backlog of initiatives by RICE = (Reach × Impact × Confidence) / Effort, turning "what should we build first" into one comparable score per item: estimate how many it reaches, how much it moves the goal, how sure you are, and what it costs, then divide and rank. The point is not the precision of any single estimate — every input is a guess — but that all items are guessed on the same definitions, so the ranking reflects relative value rather than relative volume of advocacy. RICE was built at Intercom to settle backlog fights without the loudest voice winning.
...
Before any scoring, fix five things and write them down. Locking the definitions first is what prevents a rigged score, exactly as a weighted matrix locks its weights before scoring:
...
4. **The Confidence levels.** 100% solid data, 80% some data, 50% a moonshot; below 50% means validate, do not score. This is the bias check — agree now that it will be applied, not skipped.
...
- **Calibrate before scoring.** Lock the goal, the Reach window, the Impact scale, the Confidence levels, and the Effort unit before any item is touched. Defining the scales after seeing the items is the easiest way to rig the ranking.
- **One window, one goal, one unit.** Reach on the same window, Impact on the same goal, Effort in the same currency, for every item. A factor measured differently across items is noise in the ranking, not a comparison.
- **Confidence is the bias check, never skip it.** It exists to penalize estimates that lack evidence. Drop it and RICE reverts to the loudest-voice ranking it replaced; below 50%, validate rather than score.
...
- **Sensitivity, always.** Vary Confidence and Effort and report whether the order holds. A high-rank item that depends on an optimistic estimate is the costliest thing to ship first.
- **The score sequences, it does not decide.** RICE orders a backlog by expected value per effort; strategy, dependencies, and a missing factor can still override the order.
- **Parallel where independent.** Items are estimated concurrently, then reconciled together for consistency before ranking.
