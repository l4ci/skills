...
Reference-class forecasting (Kahneman and Tversky's *inside view* versus *outside view*, operationalized for planning by Bent Flyvbjerg to expose chronic cost and schedule overruns of large projects) treats your case not as a unique story but as one instance of a class of similar past cases whose actual outcomes are the most reliable anchor you have.

It is built to defeat the **planning fallacy**: forecasting from the inside view (the specific plan, the specific team, the reasons this time will go smoothly) systematically underestimates time and cost and overstates success, because it sees the plan but not the unknown unknowns that derailed every comparable effort. The outside view corrects this by starting from the base rate: how did projects *like this* actually turn out, before you adjust for anything special about yours.
...
This move makes or breaks the method and carries a genuine tension: the class must be **similar enough** that its outcomes are relevant to your case, yet **broad enough** to contain enough cases for a stable distribution. Too narrow ("projects exactly like ours") and you have a class of one, back to the inside view; too broad ("all projects") and the base rate is noise.
...
- Construct the empirical **distribution**: at minimum the median and the range; better, the key percentiles (e.g. P10 / P50 / P90). For a yes/no outcome, the base rate is the historical success frequency.
...
The median of this distribution is the **base rate**, the anchor everything else adjusts from. Output: the distribution (median, range, percentiles) with its sample size and quality caveats.

## Stage 4: Locate the case and adjust (PARALLEL)

Place the case in the distribution and make only the adjustments the evidence actually warrants; the case is much closer to the median than the inside view will want to admit. Dispatch three critic subagents in parallel, each guarding against a different way the adjustment goes wrong:
...
1. **The forecast.** The central estimate with its interval, stated as a range ("most likely 14 months, plausibly 11 to 22"), never a false-precision point.
...
- **Anchor on the base rate, adjust second.** The class's historical outcome is the foundation; the specifics of your case are an adjustment to it, not the starting point. Start outside, then come in.
- **Your case is less special than it feels.** Almost every adjustment people want is optimistic, and almost every "this time is different" was also believed by the cases that went the usual way. Keep adjustments small, evidenced, skeptically reviewed.
- **Forecast a distribution.** The class's spread is your uncertainty. A single number throws away the most honest thing the method gives you; report a range and the contingency it implies.
- **The gap is the gift.** The distance between the inside-view estimate and the outside-view forecast is often the single most useful number produced: the optimism that would otherwise have been built silently into the plan.
