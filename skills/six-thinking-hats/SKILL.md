...
Runs Edward de Bono's Six Thinking Hats over a topic, idea, or decision, examining it in six distinct modes one at a time: White (facts), Red (feelings), Black (caution), Yellow (benefits), Green (creativity), and Blue (process control). The point is not to collect six lists but to think in parallel: everyone looks in the same direction at once instead of arguing across directions, so the topic is examined from every angle without the discussion becoming a fight.
...
The user wants to evaluate an idea, plan, or decision from all sides, run a structured deliberation, or break a deadlock where people are talking past each other. The unit is one focus question examined in full. Distinguish from a pre-mortem, which looks only at how something fails (that is the Black hat alone), and from a weighted decision matrix, which scores defined options against criteria. Six Hats is for opening a question up across every mode of thought before, or instead of, narrowing it. If there is a single clear question, use this; if the user is choosing among several options on known criteria, a scoring method fits better.
...
- **CONTEXT** (optional): the stakes, the people affected, the decision the session should inform, and any constraints. This shapes the focus question, the hat sequence, and the framing of the synthesis, not the discipline of each hat.
...
3. **Success.** Say what a good outcome of the session looks like, and confirm the conclusion is genuinely open, not a foregone decision the hats are being run to ratify.

## Stage 2: Five hats (PARALLEL)

Dispatch five subagents at once, one per substantive hat: White, Red, Black, Yellow, Green. Give each the shared framing plus its hat's discipline from the reference file. This is parallel thinking: all five look at the same focus question simultaneously, each confined to its own mode.
...
One agent puts the Blue hat back on and integrates across the five. This is where parallel thinking pays off, by weighing the directions rather than restating them:
...
2. **Weigh.** Which findings carry real weight and which are minor? Guard against Black-hat dominance: do not let the volume of risks decide the outcome on its own; set them against the Yellow value and the Green routes around them.
...
5. **Conclusion and actions.** Reach a clear conclusion or decision on the focus question, or state plainly what is still unresolved and why. List the concrete next actions that follow.
...
- **One mode at a time.** Each hat stays strictly in its lane. A risk smuggled into the facts, or a critique smuggled into an idea, breaks the method. Hold the discipline.
- **Parallel, not adversarial.** Every hat looks in the same direction at once. Separate the hat from the thinker: the same subject can be argued for under Yellow and against under Black without contradiction.
...
- **Keep the outcome open.** Do not run the hats to ratify a decision already made. The Blue hat states up front that the conclusion can change.
- **Parallel where independent.** The five substantive hats examine the question concurrently. The integration needs all five, so the closing Blue hat runs after.
