...
Administers the HEXACO personality assessment (Honesty-Humility, Emotionality, eXtraversion, Agreeableness, Conscientiousness, Openness to Experience) using the public-domain 240-item IPIP-HEXACO scales, then scores it and writes a detailed markdown report. HEXACO is the Big Five plus a sixth factor — **Honesty-Humility** — that OCEAN does not measure, and its Emotionality and Agreeableness are rotated relative to Big Five Neuroticism and Agreeableness. If the user wants the classic five-factor test instead, use [ocean-personality-test](../ocean-personality-test/).
...
- **Faithful instrument.** Use the exact items and keying from the reference file. Do not improvise items, drop items, or trim the set to a shorter form — there is no public-domain short version. The validity depends on it.
- **No leakage during testing.** Keep factor and facet labels and keying hidden until scoring, so the user answers the statement rather than the trait.
- **Honesty-Humility is the point.** It is the factor Big Five lacks; give it full weight in scoring and in the report rather than treating it as a footnote.
...
- **Honest about limits.** Always carry the non-clinical, self-report, no-norms, short-facet caveats into the report. Do not overclaim precision.
